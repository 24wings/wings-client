...
export class WorkerPagecomponent {
  ...
  selectWorkerChange(e: any) {
    if (this.workerGrid) {
      let data = this.workerGrid.instance.getSelectedRowsData()[0];
      let ip = (data.ip as string).startsWith("http://")
        ? data.ip
        : `http://` + data.ip;
    ...
    setTimeout(() => {
      this.gameGrid.instance.on("selectionChanged", () => {
        this.selectGameProcess(this.gameGrid.instance.getSelectedRowsData()[0]);
      });
    }, 1000);
  }
  ...
  ngAfterViewInit(): void {
    ...
    this.workerGrid.instance.on("selectionChanged", () => {
      // this.selectWorkerChange();
    });
  }
  ...
  goGameProcessDetailPage() {
    this.router.navigateByUrl(
      `/admin/worker/game-process-detail?pid=${
        this.selectedGameProcess.windowTitle
      }&&ip=${this.selectedGameProcess.ip}`
    );
  }
}
